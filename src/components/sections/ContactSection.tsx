...
export function ContactSection() {
  ...
  const handleFocus = (field: string) => {
    window.dispatchEvent(new CustomEvent('solaiFormFocus', { detail: { field } }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.warn('Mock submit: API contact endpoint error or local environment', err)
    }
    setLoading(false)
    setSubmitted(true)
    window.dispatchEvent(new CustomEvent('solaiFormSuccess'))
  }

  const sectionContent = (
    <div className="mx-auto max-w-5xl py-12 md:py-20 px-4 w-full">
      {/* Heading */}
